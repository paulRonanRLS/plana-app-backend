"""Handler for physical_state intent."""
...
class PhysicalStateHandler(IntentHandler):
    intent = "physical_state"

    async def handle(self, ctx: HandlerContext) -> str:
        write_capture(ctx.db, "physical_state", ctx.text)
        if ctx.claude_client is None:
            return "Noted. Is this affecting today's training?"
...
REGISTRY.register(PhysicalStateHandler())
